...
class location_name_search(Resource):
    def post(self):
        ...
        locations = []
        for document in Place_LatLong_API.find(
            {}, {"traffic_data": 0, "air_data": 0, "_id": 0}
        ):
...
class location_name_autofill(Resource):
    def post(self):
        ...
        for document in Place_LatLong_API.find({}, {"_id": 0, "id": 1, "place": 1}):
            if keyword in to_lowercase_english(document["place"]):
                locations.append({"id": document["id"], "place": 1})
        return {"count": len(locations), "keyword": keyword, "locations": locations}
...
class location_nearby(Resource):
    def post(self):
        ...
        center = Place_LatLong_API.find_one(
...
class data_current(Resource):
    def get(self, id):
        location = Place_LatLong_API.find_one(
            {"id": id},
            {
                "id": 1,
                "lat": 1,
    ...
    def post(self):
        ...
        location = data_summary.find_one(
            {"id": location_id},
            {
                ...
                "place": 1,
                str(today.date()): 1,
                str((today - timedelta(1)).date()): 1,
                str((today - timedelta(2)).date()): 1,
                str((today - timedelta(3)).date()): 1,
                str((today - timedelta(4)).date()): 1,
                str((today - timedelta(5)).date()): 1,
                str((today - timedelta(6)).date()): 1,
            },
        )
        data_day = [
            {
                ...
                "traffic_quality_index": sum(
                    location[str((today - timedelta(6 - i)).date())]["traffic_summary"]
                )
                / 24,
                "air_quality_index": sum(
                    location[str((today - timedelta(6 - i)).date())]["air_summary"]
                )
                / 24,
            }
            for i in range(0, 7)
        ...
        data_day = [
            {
                "day": i,
                "traffic_quality_index": sum(
                    location[str((today - timedelta(date_range - 1 - i)).date())][
                        "traffic_summary"
                    ]
                )
                / 24,
...
class ranking_current(Resource):
    def post(self):
        try:
            ...
            option = "both"
        locations = Place_LatLong_API.find(
            {},
            {
                "_id": 0,
                "id": 1,
        ...
        for location in locations:
            ...
            locations_traffic = sorted(
                locations_traffic, key=lambda d: d["traffic_quality_index"]
            )
            ...
            locations_air.append(
                {
                    ...
                    "air_quality_index": max(
                        calculate_aqi(
                            air_data["components"]["co"] * 0.873 * 0.001, "co"
                        ),  # Convert from milligram/m3 to ppm
                        calculate_aqi(
                            air_data["components"]["no2"] * 0.531 * 1, "no2"
                        ),  # Convert from milligram/m3 to ppb
                        calculate_aqi(
                            air_data["components"]["so2"] * 0.382 * 1, "so2"
                        ),  # Convert from milligram/m3 to ppb
                        calculate_aqi(
                            air_data["components"]["o3"] * 0.509 * 0.001, "o3"
                        ),  # Convert from milligram/m3 to ppm
                        calculate_aqi(
                            air_data["components"]["pm2_5"], "pm2_5"
                        ),  # Convert from milligram/m3 to ppm
                        calculate_aqi(
                            air_data["components"]["pm10"], "pm10"
                        ),  # Convert from milligram/m3 to ppm
                    ),
                }
            )
...
class ranking_daily(Resource):
    def post(self):
        ...
        for data_piece in data_summary.find(
            {}, {"_id": 0, "id": 1, "place": 1, today: 1, yesterday: 1}
        ):
            try:
                avg_traffic = 0
                compared_traffic = 0
                ...
                for i in range(0, 24):
                    avg_traffic += (
                        data_piece[today]["traffic_summary"][i]
                        * data_piece[today]["traffic_count"][i]
                        / traffic_count
                    )
                    compared_traffic += (
                        data_piece[yesterday]["traffic_summary"][i]
                        * data_piece[yesterday]["traffic_count"][i]
                        / traffic_count
                    )
                ...
                for i in range(0, 24):
                    avg_air += (
                        data_piece[today]["air_summary"][i]
                        * data_piece[today]["air_count"][i]
                        / air_count
                    )
                    compared_air += (
                        data_piece[yesterday]["air_summary"][i]
                        * data_piece[yesterday]["air_count"][i]
                        / air_count
                    )
                air_ranking.append(
                    {
                ...
                pass
        traffic_ranking = sorted(
            traffic_ranking, key=lambda d: d["traffic_quality_index"]
        )
        ...
        for data_piece in data_summary.find(
            ...
            try:
                if option == "traffic":
                    ...
                    for i in range(0, 7):
                        avg_traffic += sum(
                            data_piece[str((today - timedelta(6 - i)).date())][
                                "traffic_summary"
                            ]
                        ) / (24 * 7)
                    traffic_ranking.append(
...
class get_image(Resource):
    def get(self, id):
        url = Place_LatLong_API.find_one({"id": id}, {"_id": 0, "request": 1})[
            "request"
        ]
        headers = {
