...
def calculate_aqi(concentration, pollutant):
    ...
    try:
        ...
        i_high = i_low
        aqi = round(
            ((aqi_high[i_high] - aqi_low[i_low]) / (c_high[i_high] - c_low[i_low]))
...
def getAirData(path, db, collection):
    ...
    try:
        ...
        print("Error while fetching air data ", path[0], e)
        return
    ...
    try:
        ...
        hour = hcm_time.hour
        try:
            data_count = db["data_summary"].find_one({"id": path[0]})[today][
                "air_count"
            ][hour]
            data_count_array = db["data_summary"].find_one({"id": path[0]})[today][
                "air_count"
            ]
            data_summary = db["data_summary"].find_one({"id": path[0]})[today][
                "air_summary"
            ]
        except:
            db["data_summary"].update_one(
                {"id": path[0]},
                {
                    "$set": {
                        today + ".air_count": [0 for _ in range(0, 24)],
                        today + ".air_summary": [0 for _ in range(0, 24)],
                    }
                },
            )
            data_count_array = db["data_summary"].find_one({"id": path[0]})[today][
        ...
        data_summary[hour] = data_summary[hour] * data_count + max(
            calculate_aqi(
                insert_data["components"]["co"] * 0.873 * 0.001, "co"
            ),  # Convert from miligram/m3 to ppm
            calculate_aqi(
                insert_data["components"]["no2"] * 0.531 * 1, "no2"
            ),  # Convert from miligram/m3 to ppb
            calculate_aqi(
                insert_data["components"]["so2"] * 0.382 * 1, "so2"
            ),  # Convert from miligram/m3 to ppb
            calculate_aqi(
                insert_data["components"]["o3"] * 0.509 * 0.001, "o3"
            ),  # Convert from miligram/m3 to ppm
            calculate_aqi(
                insert_data["components"]["pm2_5"], "pm2_5"
            ),  # Convert from miligram/m3 to ppm
            calculate_aqi(
                insert_data["components"]["pm10"], "pm10"
            ),  # Convert from miligram/m3 to ppm
        )
        data_count += 1
...
if __name__ == "__main__":
    ...
    # Fetch all paths in a single query
    paths = list(
        database[collection].find(
            {}, {"_id": 0, "id": 1, "place": 1, "request": 1, "lat": 1, "long": 1}
        )
    )
    ...
    threads = []
    for path in paths:
        thread = threading.Thread(target=getAirData, args=(path, database, collection))
